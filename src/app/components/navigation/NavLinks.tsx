...
export const NAV_ITEMS: NavItem[] = [
  ...
  { label: "Posts", href: "/pages/posts" },
  { label: "Join Us!", href: "/pages/contact", isCTA: true },
];
...
export interface NavLinksProps {
  ...
  /** Optional: use anchor instead of Next Link (e.g. for Radix NavigationMenu.Link) */
  asAnchor?: boolean;
}
...
export const NavLinks: React.FC<NavLinksProps> = ({
  ...
  const listClassName = `NavLinksList NavLinksList--${direction}`;

  const linkContent = (item: NavItem) => {
    /* In the mobile drawer (vertical) show "Home" text; on desktop (horizontal) show logo for first item */
    if (item.isLogo && direction === "horizontal") {
      return <LogoIcon />;
    }
    return <>{item.label}</>;
  };
  ...
  return (
    <ul className={listClassName} role="list">
      {NAV_ITEMS.map((item) => {
        ...
        const className = [
          "NavLink",
          item.isLogo && direction === "horizontal" && "NavLink--logo",
          item.isCTA && "NavLink--cta",
